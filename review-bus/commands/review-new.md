---
description: Create a new review_bus topic folder (topic.md + status.json) from templates
argument-hint: "<slug> [priority p0|p1|p2|p3]"
---

Create a new review_bus topic.

Arguments: `$ARGUMENTS` — first token is the topic `<slug>` (kebab-case), optional second token is priority (default `p2`).

Steps:

1. Resolve the review_bus root (the directory containing `run_auto.ps1`; default `llm_wiki/review_bus`).
...
Follow the rules in the review_bus `README.md`. Do not call any `codex:*` skill to write another agent's round.
