...
**라운드 캡**: 번호 문서(`NNN_*.md`)가 캡(코디네이터 `-MaxNumberedDocs` 기본 7, 토픽별 `max_rounds` 우선)에 도달하면 다음 턴은 강제 JUDGE로 전환되어 기존 라운드만으로 `decision.md`를 작성합니다 — 핑퐁 토론도 반드시 종결됩니다. **1라운드 = 번호 문서 1개**(design=1, attack=2, …). `max_rounds`는 매 폴마다 재독되므로 진행 중 토픽에 `update_status.ps1 -Set @{ max_rounds = N } -Force`로 바꾸면 **다음 턴부터 즉시 적용**됩니다(재시작 불요).
